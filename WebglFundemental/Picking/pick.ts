...
function main() {
  ...
  // and gl.bufferData
  const sphereBufferInfo = twgl.primitives.createSphereBufferInfo(gl, 10, 12, 6);
  const cubeBufferInfo   = twgl.primitives.createCubeBufferInfo(gl, 20);
  const coneBufferInfo   = twgl.primitives.createTruncatedConeBufferInfo(gl, 10, 0, 20, 12, 1, true, false);
