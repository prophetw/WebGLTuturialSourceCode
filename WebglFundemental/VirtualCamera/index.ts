...
function main() {
  ...
  const vertexColorProgramInfo = twgl.createProgramInfo(gl, [Shader3DVS, Shader3DFS]);
  ...
  function createCameraBufferInfo(gl: WebGLRenderingContext, scale = 1) {
    // first let's add a cube. It goes from 1 to 3
    // because cameras look down -Z so we want
  ...
  const canvas = gl.canvas as HTMLCanvasElement
    ...
    const effectiveWidth = canvas.clientWidth / 2;
    const aspect = effectiveWidth / canvas.clientHeight;
    const near = 1;
    const far = 2000;
