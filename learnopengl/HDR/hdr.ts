import LightFS from './lighting.fs'
import LightVS from './lighting.vs'
import HdrFS from './hdr.fs'
import HdrVS from './hdr.vs'
...
function main() {
  ...
  // init program
  const pbrProgramInfo = twgl.createProgramInfo(gl, [HdrVS, HdrFS])
  const pbrProgram = pbrProgramInfo.program
  const eqToCubeProgramInfo = twgl.createProgramInfo(gl, [LightVS, LightFS])
